...
int main( void )
{
	...
	for (page_num = 0; page_num < PAGE_COUNT; page_num++, data_addr += 0x100)
	{
		...
		/* Generate Parity Codewords for parity array */
...
int init( void )
{
	Status = SetupInterruptSystem(&Spi,
				...
				TIMER_CNTR_0);
	if (Status != XST_SUCCESS){
		printf("ERROR: unable to initialize interrupts\n");
		return XST_FAILURE;
	}

	Status = TimerInit(&InterruptController,
				&TimerCounterInst,
				TMRCTR_DEVICE_ID,
				TMRCTR_INTERRUPT_ID,
				TIMER_CNTR_0);
	...
	Status = SpiFlashInit(SPI_DEVICE_ID, &Spi);
	if (Status != XST_SUCCESS) {
		printf("ERROR: Flash Initialization\n");
		return XST_FAILURE;
	}

	/* Initialize BCH Control */
	if ( !(BCHptr = init_bch(M, T, 0)) ) {
		printf("ERROR: BCH Initialization\n");
		return XST_FAILURE;
	}
	return XST_SUCCESS;
}
 ...
 */
int SpiFlashInit	( u16 DevID, XSpi *SpiPtr )
{
	int Status;
...
int SpiFlashClear	( XSpi *SpiPtr, u32 Addr, int cmd )
{
	...
	switch (cmd) {
		...
		case (2) :
			if (SpiFlashBulkErase(SpiPtr) != XST_SUCCESS) {
				printf("Bulk Erase Error!\n");
				return XST_FAILURE;
			}
	}
	return XST_SUCCESS;
 ...
 */
int FlashWrite	( XSpi *SpiPtr, u32 Addr, u32 ByteCount, u8 *data )
{
	int Status;
	...
	Status = SpiFlashWrite(SpiPtr, Addr, ByteCount, COMMAND_QUAD_WRITE, data);
	if (Status != XST_SUCCESS) {
		printf("Flash Write Error!\n");
		return XST_FAILURE;
	}

	printf("Writing to %lu: ", Addr);
	for (Status = 0; Status < ByteCount; Status++) { printf(" %d", data[Status]); }
	printf("\n");
 ...
 * @note	None
 */
int FlashRead	( XSpi *SpiPtr, u32 Addr, u32 ByteCount, u8 *data )
{
	int Status; /*, Index; */
	...
	memcpy(data, ReadBuffer+READ_WRITE_EXTRA_BYTES, ByteCount);
	return XST_SUCCESS;
}

int SpiFlashExit	( XSpi *SpiPtr ) {
	int Index;
	for(Index = 0; Index < PAGE_SIZE + READ_WRITE_EXTRA_BYTES; Index++) {
		ReadBuffer[Index] = 0x0;
	}

	if (SpiFlashClear(SpiPtr, 0x00, 2) != XST_SUCCESS) {
		printf("CLEAR() ERROR!\n");
		return XST_FAILURE;
	}
						 ...
						 u8 TmrCtrNumber)
{
	 ...
	 */
	Status = XIntc_ConnectFastHandler(&InterruptController, IntrId,
...
struct bch_control *init_bch(int m, int t, unsigned int prim_poly)
{
    ...
    if (bch == NULL) {
        printf("Unable to initialize bch\n");
    	goto fail;
    }
    ...
    if (err) {
        printf("Error Allocating BCH\n");
    	goto fail;
    }
    err = build_gf_tables(bch, prim_poly);
    if (err) {
        printf("Error Building GF tables\n");
    	goto fail;
    }
    /* use generator polynomial for computing encoding tables */
    genpoly = compute_generator_polynomial(bch);
    if (genpoly == NULL) {
    	printf("Error computing GenPoly\n");
        goto fail;
    }
    ...
    if (err) {
        printf("Error Building deg2 base\n");
    	goto fail;
    }
